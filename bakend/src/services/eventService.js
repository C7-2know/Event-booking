...
class EventService {
    ...
    async getAllEvents() {
        const events = await Event.find().populate("createdBy", "name email profilePicture");
        return events;
    }

    async getEventById(eventId) {
        const event = await Event.findById(eventId).populate("createdBy", "name email profilePicture");
        if (!event) {
