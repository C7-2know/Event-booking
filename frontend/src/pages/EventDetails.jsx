...
import { CalendarDays, Clock3, MapPin, Users, DollarSign, ArrowLeft, Heart, Share2, Check } from "lucide-react";
...
export default function EventDetails() {
  ...
  if (!event) {
    return <div className="max-w-7xl mx-auto py-24 text-center text-gray-400">Loading event…</div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto py-6">
      <Link to="/" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-800">
        <ArrowLeft size={16} /> Back to events
      </Link>

      <div className="relative mt-4">
        <img src={event.image} alt="" className="h-[270px] md:h-[420px] w-full object-cover rounded-xl" />
        <div className="absolute top-4 right-4 flex gap-2">
          ...
          >
            <Heart size={16} className={saved ? "fill-emerald-700 text-emerald-700" : "text-gray-600"} />
          </button>
          <button aria-label="Share event" className="h-9 w-9 rounded-full bg-white/90 flex items-center justify-center shadow">
      ...
      <div className="grid gap-12 p-10 lg:grid-cols-[1.25fr_.75fr] lg:items-center">
        ...
        <article>
          ...
          <h1 className="mt-4 text-3xl md:text-4xl font-semibold text-gray-900">{event.title}</h1>
          <p className="mt-4 max-w-2xl text-gray-600 leading-7">{event.description}</p>
          ...
          <div className="mt-10 border-t border-gray-100 pt-6">
            <div className="grid gap-5 sm:grid-cols-1">
              ...
              <Info icon={DollarSign} label="Price" text={`$${event.price.toFixed(2)}`} />
              <Info icon={Users} label="Seats Left" text={`${event.remaining} / ${event.capacity}`} />
            </div>
          </div>
          ...
          <div className="bg-green-50/50 mt-10 rounded-xl p-5">
            <div className="mt-8 rounded-xl bg-emerald-50/60 p-5">
                ...
                <p className="mt-2 text-sm text-gray-600 leading-6">{event.about ?? event.description}</p>
            </div>

            {event.organizer && (
                <div className="mt-8">
                    <h2 className="text-sm font-semibold text-gray-900 mb-3">Organizer</h2>
                    <div className="flex items-center gap-3">
                        ...
                        <Link to={`/organizers/${event.organizer.id}`} className="text-xs text-green-700 hover:underline">
